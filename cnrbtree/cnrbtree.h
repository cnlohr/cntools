...
CNRBTREE_GENERIC_DECORATOR cnrbtree_generic_node * cnrbtree_generic_get2( struct cnrbtree_generic_t * tree, cnrbtree_generic_compare fn, const void* key, int approx );
CNRBTREE_GENERIC_DECORATOR cnrbtree_generic_node * cnrbtree_generic_get( struct cnrbtree_generic_t * tree, cnrbtree_generic_compare fn, const void* key );
...
CNRBTREE_GENERIC_DECORATOR cnrbtree_generic_node * cnrbtree_generic_access( struct cnrbtree_generic_t * tree, cnrbtree_generic_copykey copy_fn, cnrbtree_generic_compare cmp_fn, const void* key, int node_size )
	{
		const cnrbtree_generic_node * nil = cnrbtree_generic_nil();
		/* This function could utilize cnrbtree_generic_get2 but would require an extra compare */
		cnrbtree_generic_node * tmp = 0;
		cnrbtree_generic_node * tmpnext = 0;
		tmp = tree->node;
		int cmp = 0;
		while( tmp != nil )
		{
...
CNRBTREE_GENERIC_DECORATOR cnrbtree_generic_node * cnrbtree_generic_get( struct cnrbtree_generic_t * tree, cnrbtree_generic_compare fn, const void* key )
{
    return cnrbtree_generic_get2( tree, fn, key, 0);
}
...
#define CNRBTREETEMPLATE( key_t, data_t, comparexy, copykeyxy, deletekeyxy ) \
	...
	{\
         return (cnrbtree_##key_t##data_t##_node *)cnrbtree_generic_get2((struct cnrbtree_generic_t *)tree, &cnrbtree_##key_t##data_t##_compare, &key, approx);                                                                    \
	}\
	\
	...
	{\
		cnrbtree_##key_t##data_t##_node * nil = cnrbtree_##key_t##data_t##_nil(); \
	...
	CNRBTREE_TEMPLATE_DECORATOR __attribute__((noinline)) void cnrbtree_##key_t##data_t##_destroy_node_internal( cnrbtree_##key_t##data_t * tree, cnrbtree_##key_t##data_t##_node * node ) \
	{\
